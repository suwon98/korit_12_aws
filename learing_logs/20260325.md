...
- localhost로 실행할 때 컴퓨터 한 대에서 backend 서버와 DB를 동시에 돌렸는데, 그 말은 EC2 내부에 DB를 설치해서 사용하는 방법이 있지 않을까요? -> 가능.

- 이유는 이하에서 설명

RDS는 이하의 세 가지 옵션을 설정할 필요가 있습니다.
1. 엔진 유형 : DB 종류라고 생각하면 편합니다. 주요 데이터베이스의 엔진은 MySQL, MariaDB, Amazon Aurora 등.
2. 인스턴스 클래스 : 컴퓨터 성능을 의미합니다. 우리는 프리티어 썼던거 EC2에서의 인스턴스 유형과 유사한 의미.
3. 스토리지 : RDS 인스턴스도 컴퓨터이기 때문에 저장 공간이 존재합니다. EC2와 용어 동일.

## RDS를 사용하는 이유
1. EC2로 백엔드 배포를 하고, DB는 본인 컴퓨터에 설치해서 쓰는 방법도 가능.
2. EC2에 백엔드 및 DB를 배포하는 방법
  - 반드시 RDS를 활용할 필요는 없습니다. 토이 프로젝트의 경우에는 가능할 것 같습니다(대신 create-drop 해두면 더비 데이터를 미리 많이 CommandLineRunner를 통해 집어넣어둬야겠네요). 실무에서는 권장하지 않습니다. 만약에 백엔드 서버에 장애 발생 시에 EC2 인스턴스에 이상이 생길 경우 DB도 영향을 받을 수 있기 때문입니다.
  
## 저희 RDS 적용 아키텍쳐 구성
![alt text](image-12.png)
- Aurora and RDS -> 데이터베이스
- public access -> Yes -> 그래야 개발환경이나 로컬에서도 바로 RDB 접근할 수 있음
- 나머지는 free tier로 잡아놨고
- 마스터 ID 및 패스워드 설정 했습니다

## RDS 보안그룹 설정
- EC2로 가서 보안그룹
- 이름, 설명 추가
- 인바운드 규칙 추가 -> 사용하는 DB 선택 -> 소스 anywhere
...
1. 이하의 속성들 전부 utf8mb4 로 설정하기 -> 이미지 등 다 저장하려고
...
2. 이하의 속성들 전부 utf8mb4_unicode_ci 로 변경하기 -> unicode_ci : 정렬 / 비교 방식 명시
...
## RDS로 접속하기
- 컴퓨터에 DBeaver 설치 여부 확인
- SQL 학습할 때 있었으니까 SQL repository 내에 있겠네요.


- instagram-db 인스턴스로 들어가서 연결 부분을 엔드포인트로 지정
- dbeaver 켠 다음에 데이터베이스 연결 -> MySQL 선택 후에 servername에 aws 엔드포인트를 붙혀넣었고,  username / password를 인스턴스 설정 시 만들었던 master id 와 master password를 입력했습니다.
...
클론 이후에 application.yml 파일 수정 했습니다.
```bash
...
- 이상의 `nohup`을 사용하게 되면 터미널을 나가더라도 백그라운드에서 여전히 실행되고 있어(즉, port를 점유하고 있어) 다시 빌드하고 재실행을 시키려고 할 때 포트 점유 로그가 뜨면서 싫애이 되지 않을 수 있습니다.
...
켤 때 백엔드에서 `url: jdbc:mysql://___________:3306/instagram`로 이름을 미리 instagram으로 설정을 해놔서 데이터베이스에 동일한 명의 데이터베이스가 있어야 실행이 됨.

## 안전 삭제
- EC2 - RDS 간의 연결은 : application.yml을 통해서만 이루어져있고 인스턴스 추가 등 AWS에서 설정한 적이 없습니다.

- RDS 삭제를 시도하면 자동 백업 관련 하는데 이러면 삭제해도 돈 나감
- RDS를 삭제 했다면 -> Security Group / Parameter Group

근데 삭제하는데 시간 좀 걸림. EC2 삭제
- 그럼 여기 딸려있는 얘들도 이제 삭제 가능 -> Security Group / key-pair / 탄력적 IP

나머지 다 삭제 하면 됨

# 내일
EC2 생성 -> springboot 프로젝트 clone (8080수정 안함)
RDS 생성 -> DBeaver로 로컬 DBMS와 AWS RDS 간의 연결 형성
springboot project와 RDS를 연결
탄력적 IP 발급 -> springboot project와 연결
도메인 발급 -> 탄력적 IP 와 연결
EC2 내부에 nginx 설치
CertBot 설치
https 설정

특정 상황 이후에 postman 통한 /boards 엔드포인트 요청한거 어디까지 완성하면 가능한지 고민 해볼 것